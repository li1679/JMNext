package com.par9uet.jm.domain.notification
...
object NotificationIcon {
    ...
    fun init(smallIconRes: Int) {
        resId = smallIconRes
    }

    fun small(): Int = if (resId != 0) resId else android.R.drawable.stat_sys_download
}
...
fun showUpdateDownloadedNotification(
    ...
    // 不硬引用 MainActivity：服务层不该依赖界面层的具体 Activity，
    // 用启动 Intent 打开应用即可，路由信息通过 extra 传递。
    val intent = (
    ...
    val notification = NotificationCompat.Builder(context, UPDATE_DOWNLOADED_CHANNEL_ID)
        .setSmallIcon(NotificationIcon.small())
