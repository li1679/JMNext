...
class CategoryComicPagingSource(
    ...
    private val tagFilter: ComicTagFilter = ComicTagFilter(repository),
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, Comic> {
        ...
        return when (val result = repository.getCategoryComics(page, filter)) {
            ...
            is NetWorkResult.Success -> when (val filtered = tagFilter.filter(result.data.comics, blockedTags)) {
